...
from others_convert_0501 import (
    process_other_excel_files,
    sheet_name_map as others_sheet_map,
)
from excelToSpecialTxt_0501 import (
    convert_excel_to_txt_integrated,
    sheet_name_map as credit_limits_sheet_map,
)
...
def main(argv):
    ...
    try:
        opts, args = getopt.getopt(
            argv, "hi:m:d:p:", ["help", "input=", "model=", "date=", "ip="]
        )
    ...
    for opt, arg in opts:
        if opt in ("-h", "--help"):
            print(
                "Usage: data_conversion.py -i <inputfile> -m <model> -d <date> -p <ip>"
            )
            sys.exit()
        elif opt in ("-i", "--input"):
            input_file = arg
