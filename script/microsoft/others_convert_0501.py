...
def format_date_columns(df, date_columns):
    for col in date_columns:
        ...
        if not format_type.isin([False, None]).any():
            df[col] = df[col].apply(lambda x: datetime.strptime(str(x), format_type.iloc[0]).strftime('%Y%m%d'))
    return df
...
def process_excel_file(file_path, output_path):
    ...
    filtered_sheet_names = [name for name in xls.sheet_names if not re.search('[\u4e00-\u9fff]', name)]
    ...
    for sheet_name in filtered_sheet_names:
        ...
        date_columns = [col for col in df.columns if
                        any(keyword in col for keyword in ['日期', '日', '时间', '期限', '时点'])]
